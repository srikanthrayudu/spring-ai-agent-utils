...
import org.springaicommunity.agent.memory.risk.AlertConsolidationEngine.ConsolidatedAlert;
import org.springaicommunity.agent.memory.risk.AlertConsolidationEngine.ConsolidationResult;
...
public class InteractiveDashboardGenerator {
    ...
    public String generate(List<UnifiedIdentity> identities, List<KnowledgeUnit> risks,
        ...
        if (exceptions != null) {
            var riskEngine = new org.springaicommunity.agent.memory.risk.DefaultRiskDetectionEngine();
        ...
        template = template.replace("{{OVERVIEW_CHARTS}}", "");
