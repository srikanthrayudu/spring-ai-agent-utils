...
import org.springaicommunity.agent.memory.synthesis.KeywordPatternDiscoveryEngine;
import org.springaicommunity.agent.memory.synthesis.PatternDiscoveryEngine;
...
import org.springaicommunity.agent.memory.validation.KnowledgeValidator;
...
public class KnowledgeEvolutionPipeline {
	...
	public KnowledgeUnit addEvidence(String id, String newEvidence) {
		...
		List<org.springaicommunity.agent.memory.model.Evidence> updatedEvidence = existing.evidence() != null ? new ArrayList<>(existing.evidence()) : new ArrayList<>();
		boolean found = updatedEvidence.stream().anyMatch(e -> e.description().equals(newEvidence));
		if (!found) {
			updatedEvidence.add(new org.springaicommunity.agent.memory.model.Evidence("legacy", newEvidence, 0.5));
		}
